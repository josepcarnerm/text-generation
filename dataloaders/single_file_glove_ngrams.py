...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        random.seed(index)
        ngram_offset = self.opt.ngrams - 1
        start_index = random.randint(0, self.len - self.opt.sentence_len - ngram_offset)
        end_index = start_index + self.opt.sentence_len + 1 + ngram_offset
        sentence = self.words[start_index:end_index]
        return sentence

    def __len__(self):
        return self.len - self.opt.sentence_len - (self.opt.ngrams-1)
