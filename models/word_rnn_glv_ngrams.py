...
class Model(nn.Module):
    ...
    def from_string_to_tensor(self, sentence):
        tensor = torch.LongTensor(len(sentence), self.opt.ngrams)
        for i in range(len(sentence) - (self.opt.ngrams - 1)):
            for j in range(self.opt.ngrams):
                try:
                    tensor[i, j] = self.word2idx[sentence[i+j]]
                except:
                    tensor[i, j] = self.word2idx['unk']
        return tensor
    ...
    def get_input_and_target(self, batch):
        ...
        for i, sentence in enumerate(batch):
            inp[:, [i+j for j in range(self.opt.ngrams)]] = self.from_string_to_tensor(sentence)
            target[:, [i+j for j in range(self.opt.ngrams)]] = self.from_string_to_tensor(sentence)
        inp = inp[:, :-self.opt.ngrams]
        target = target[:, self.opt.ngrams:]
        inp = Variable(inp)
    ...
    def init_hidden(self, batch_size):
        ...
        if self.opt.rnn_type.lower() is 'gru':
            return zeros(gpu=is_remote(), sizes=(self.opt.n_layers_rnn, batch_size*self.opt.ngrams, self.opt.hidden_size_rnn))
        elif self.opt.rnn_type.lower() is 'lstm':
            return (zeros(gpu=is_remote(), sizes=(self.opt.n_layers_rnn, batch_size*self.opt.ngrams, self.opt.hidden_size_rnn)),
                    zeros(gpu=is_remote(), sizes=(self.opt.n_layers_rnn, batch_size*self.opt.ngrams, self.opt.hidden_size_rnn)))
