...
class Model(WordRNNModel):
    ...
    def __init__(self, opt):
        ...
        self.submodules = self.submodules + [self.encoder_topic, self.topic_criterion]
    ...
    def select_topics_closest_word(self, batch):
    ...
    def evaluate(self, batch):
        ...
        for w in range(self.opt.sentence_len):
            ...
            topic_loss += self.topic_criterion(output.view(self.opt.batch_size, -1), topic[:, 0])
            # import pdb; pdb.set_trace();
            # output_topic, _ = self.select_topics(output)
            ...
            # loss_topic_weights = loss_topic_weights.cuda() if is_remote() else loss_topic_weights
            # loss_topic_total_weight += loss_topic_weights
            # loss_topic += self.closeness_to_topics(output, topic) * loss_topic_weights
        ...
        return self.opt.loss_alpha*loss + (1-self.opt.loss_alpha)*topic_loss
        # return self.opt.loss_alpha*loss_reconstruction + (1-self.opt.loss_alpha)*loss_topic
